...
def retinex_adjust(nimg):
    ...
    sum_g = np.sum(nimg[1])
    max_g = nimg[1].max()
    #sum_r2* x0 + sum_r x1 = sum_g and
    # max_r2* x0 + max_r x1 = max_g and
...
class TestToolSet(unittest.TestCase):
    ...
    def test_retinix(self):
        img  = image_wrap.openImageFile('tests/images/test_project4.jpg')
        img_ajdust = retinex_with_adjust(img.image_array)
        image_wrap.ImageWrapper(img_ajdust).save('foo4.png')