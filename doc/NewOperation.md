...
* donorVideoSegments = The list video and audio donor segments. NOTE: 255 = donated, 0 = not.
* targetMaskImage = The ImageWrapper target mask image (aligned to target
  final node). NOTE: 255 = manipulated, 0 = not.
* donorMaskImage= The ImageWrapper donor mask image (aligned to donor). NOTE: 255 = donated, 0 = not.
* targetMaskFileName = The file name of the target mask image (aligned to
  target final node). NOTE: 255 = manipulated, 0 = not.
* donorMaskFileName = The file name of the donor mask image (aligned to
  donor). NOTE: 255 = manipulated, 0 = not.
* targetVideoSegments = The list video and audio final node segments. NOTE: 0 = manipulated, 255 = not.
...
Probe image masks differ from link masks in that white (255) indicates manipulation. Many conversions require a 'high' value to indicate area of interest.

Probe video masks match their associated link masks to reduce to conversion time, as videos currently have minimal spatial conversion capability (e.g. global crop, resize).

Probe masks match the orientation as described by the width and height of the image or video, not the Orientation meta-data.  Thus, if an image is aligned 4000x2000 with a Orientation of 90 CW, then the mask is also 4000x2000 without rotation.  
