...
from maskgen.ui.description_dialog import DescriptionCaptureDialog, createCompareDialog,ValidationListDialog,\
    ItemDescriptionCaptureDialog
from maskgen.scenario_model import Modification
from maskgen.software_loader import extract_default_values
...
class MaskGraphCanvas(tk.Canvas):
    ...
    def onNodeButtonPress(self, event):
        ...
        if cursor == 'crosshair':
            ...
            startIm, startFileName  = self.scModel.getImageAndName(self.scModel.start)
            destIm, destFileName = self.scModel.getImageAndName(nodeId)
            if destFileName is None:
                self.itemToNodeIds.pop(item)
                self.clear()
                self.update()
                return
            file_without_path = os.path.split(destFileName)[1]
            ok = False
            if self.crossHairConnect:
                if nodeId == self.scModel.start:
                    ...
                    result = self.scModel.getCreatingOperation(nodeId)
                    if result is not None:
                        args = result[1].getDonorProcessor(self.scModel.getLinkTool(self.scModel.start, nodeId).getDefaultDonorProcessor())(
                            self.scModel.getGraph(),
                            self.scModel.start,
                            nodeId,
                            result[0],
                            (startIm, startFileName),
                            (destIm, destFileName)).arguments()
                        if len(args) > 0:
        ...
        if (item is not None):
            if self.itemToNodeIds.has_key(item):
                ...
                if e is not None:
                    ...
                    eventname = 'rcEdge' if self.scModel.isEditableEdge(e[0], e[1]) else (
                        'rcDonorEdge' if self.scModel.isDonorEdge(e[0], e[1]) else 'rcNonEditEdge')
            self._mark(item)
