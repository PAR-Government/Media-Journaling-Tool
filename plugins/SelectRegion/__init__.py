...
def transform(img, source, target, **kwargs):
    ...
    min_size = max(0.05*denoise_img.shape[0]*denoise_img.shape[1], math.ceil(sigma * 10.0) * 10)
...
def operation():
    return {'name': 'SelectRegion',
            ...
            'arguments': {'alpha': {'type': "yesno",
                          ...
                          'op': {'type': 'list', 'values': ['slic', 'felzenszwalb'], 'description': 'selection algorithm to use'}},
            'transitions': [
                'image.image'
            ]
            }
